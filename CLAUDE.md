...
- `.claude/hooks/session-start.sh` links `pre-commit`/`pre-push` and installs `cargo-deny` at
  session start — see AGENTS.md §3 for why nothing on its list needs a manual run.
