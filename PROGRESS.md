...
| F8 | High | `DefaultHasher` unstable on the wire | ✅ | #111 — wire fingerprint is BLAKE3; closed the other half by owning the *input* encoding too (`rsos::encoding`), and `version_hash` now derives from it instead of `DefaultHasher` — see §4.x below |
...
- ✅ **Canonical fingerprint encoding** — element fingerprints no longer take their bytes from
  `std::hash::Hash`. `rsos` gained `rsos/src/encoding.rs`: an injective, length-prefixed
