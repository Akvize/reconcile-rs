...
from `std::hash::Hash`; `rsos` now owns a canonical serde encoding (`rsos::encoding`, §3.10) and
...
`rsos::encoding` closes that: a `serde::Serializer` writing an injective, length-prefixed byte
