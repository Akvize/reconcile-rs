...
- **Storage** — `FingerprintTree`: an ordered map that also maintains, for every subtree,
...
|---|---|
| `rsos/src/hrtree.rs`, `rsos/src/hrtree_iter.rs` (standalone `rsos` crate, migration step 6 Step A, done) | `FingerprintTree`: ordered map + range-fingerprint data structure and its iterators, plus the `Rsos<K, V>` trait (Def. 3.9) it implements. |
...
|---|---|
| `rsos/src/hrtree.rs`, `rsos/src/hrtree_iter.rs`, `rsos/src/fingerprint.rs` (standalone `rsos` crate) | none — enforced today by the crate's own minimal `Cargo.toml` dependency list, not just by grep. |
...
- **Internal mechanism, currently public (2).** `HashRangeQueryable` (`diff.rs:30`) and `Diffable`
  ...
  real implementation (`FingerprintTree`). They are exposed through `pub mod diff` (`lib.rs:30`), placing the
...
```
  ...
  └───────────┘                 ┌──────────────────────────────────────────────────────────┐
        ...
        └───────────────────────│  tombstone lifecycle · FingerprintTree + Fingerprint (mechanism)    │
...
- the `FingerprintTree` and `Fingerprint` (the storage and range-hash mechanism),
...
tradeoff to explain — they are simply an internal mechanism, the same as `FingerprintTree`. `Clock` stays
...
Today the engine keeps a second tree `FingerprintTree<K, V::Projected>` fed through the `Projectable` trait
...
`FingerprintTree<K, State<V>>`; the `Projectable` trait and `ValueOnly` type are dissolved. The two hashes
...
range-hash querying becomes inherent methods on the concrete `FingerprintTree`, and `start_diff` / `diff_round`
...
reconcile-core   // DOMAIN + PORTS: Clock, Persistence (traits);
                 //   Entry / State, Timestamp, Fingerprint, FingerprintTree, the diff algorithm, LWW.
...
| `HashRangeQueryable`, `Diffable` (public) | inherent `FingerprintTree` methods + `pub(crate)` diff functions |
...
2. ✅ **Dissolve the diff traits** — remove `HashRangeQueryable` / `Diffable`; move `start_diff` /
   `diff_round` verbatim to `pub(crate)` functions over `FingerprintTree`.
