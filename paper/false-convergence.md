...
Independent of publication, and mostly **landed**:

- `SOTA.md` §2.1 — the exact-count coverage (§5 here: what `f_p = id` catches with probability 1 and
  what it does not) is recorded in **#344**.
- `SOTA.md` §2.4 P0-1 — the "width settles only the honest model, Wagner-breakable, keyed-lift fix"
  caveat (§6.5 here) is recorded in **#344**, pointing at #337 rather than restating the figures.
- `rsos/src/fingerprint.rs` — the module doc, that non-`GF(2)`-linearity is *insufficient* against a
  chosen-input adversary, is corrected in **this PR** (the code-site caveat).
- The keyed-lift fix is **#337**, which now carries two costed options (keyed lift; ECMH combiner) —
  a maintainer decision (§7: a combiner swap is a wire break, and `rsos` holds no key).
