...
- **vs FingerprintTreeMap:** **it is the same design**, and its combiner is literally ours —
  ...
  - **The comparison map is exact here, probabilistic there.** Negentropy's `f_p` is
    ...
    away entirely; comparing `(count, Σ mod 2^τ)` would keep it for the price of a varint. That same
    split is a **policy** signal and not only a correctness one:
    [#318](https://github.com/Akvize/reconcile-rs/issues/318)'s divergence-adaptive fan-out reads the
    count delta and goes blind on exactly the balanced case, so one workload question — *what
    fraction of divergence is count-unbalanced* — settles both at once.
